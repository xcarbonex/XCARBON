...
import { Drawer, SelectField, Input, Typography, Button, Table, useToast } from "@/components";
...
const DepositDrawer: React.FC<DepositDrawerProps> = ({ isOpen, onClose }) => {
  ...
  const handleFiatDeposit = async () => {
    try {
      ...
      toast.success("Deposit Initiated", "Redirecting to payment processor...");
      onClose();
    } catch (error) {
  ...
  const handleCryptoDeposit = async () => {
    try {
      ...
      toast.success(
        ...
        "Please send your crypto to the displayed address."
      );
    } catch (error) {
      console.error("Crypto deposit error:", error);
      toast.error("Failed", "Failed to generate deposit address. Please try again.");
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const copyToClipboard = async (text: string) => {
    try {
      ...
      toast.error("Failed", "Failed to copy address");
    }
  };

  return (
    <Drawer isOpen={isOpen} onClose={onClose} title="Deposit Funds" size="xl" anchor="right">
      <div className="space-y-6 text-tbase">
        {/* Header */}
        <div className="bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 p-4 rounded-lg">
                ...
                <FormikForm className="space-y-6">
                  ...
                  {values.asset && values.network && (
                    <>
                      ...
                      {/* QR Code */}
                      <div className="border border-border rounded-md p-8 text-center bg-white dark:bg-neutral-900">
                      ...
                      <div className="flex gap-3 p-4 border border-warning bg-warning/10 rounded-lg">
                        ...
                        <div>
                          <Typography variant="body2">
                            ...
                            {NETWORKS.find((n) => n.value === values.network)?.label} to this
                            address. Sending unsupported tokens or using the wrong network may
