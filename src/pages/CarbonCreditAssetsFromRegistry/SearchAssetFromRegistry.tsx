...
import CarbonCreditList, { CarbonCredit } from "./CarbonCreditList";
...
const SearchAssetFromRegistry: React.FC = () => {
  const registryFormRef = useRef<FormikProps<FormValues> | null>(null);
  ...
  const searchValidationSchema = Yup.object().shape({
    ...
    assetReferenceType: Yup.string().required("Asset Reference Type is required"),
  ...
  const handleSearchSubmit = (values: FormValues) => {
    setLastSearchCriteria({
      registry:
        registryOptions.find((opt) => opt.value === values.registry)?.label || values.registry,
      assetReferenceType:
    ...
    if (values.assetReferenceType === "registryURL") {
      ...
      searchRegistry(values.registry, values.assetReferenceType, "", values.reference);
    }
  };

  const handleSelectCarbonCredit = async (selectedCredit: unknown, registry: string) => {
    await fetchCarbonCreditById(registry, (selectedCredit as { id: string }).id);
  };
    ...
    return (
      <div className="bg-[#4C6663] text-white p-4 rounded-xl mb-4 mt-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
          ...
          <p>
            <strong>Asset Reference Type:</strong> {lastSearchCriteria.assetReferenceType}
          </p>
  ...
  const handleSearchQuery = (query: string, values: FormValues) => {
    setSearch(query);
    searchRegistry(values.registry, values.assetReferenceType, values.reference, query);
  };
      ...
      <div className="bg-white transition-all duration-fast dark:bg-[#141517]">
        <div className="space-y-5 text-black dark:text-[#FFFFFF]/80 ">
          <Form
            ...
            ref={registryFormRef as any}
            initialValues={filteredPamaeter}
            validationSchema={searchValidationSchema}
            onSubmit={(values) => handleSearchSubmit(values as FormValues)}
          >
                ...
                <>
                  ...
                  >
                    <div
                      ...
                      className="space-y-1 py-2 flex flex-nowrap cursor-pointer justify-start gap-3 items-center"
                    >
                      <button
                        onClick={() => navigate(-1)}
                        className="flex items-center p-2 text-[#949494] hover:text-[#C2A57B] transition-colors border border-[#363638] rounded-md hover:border-[#C2A57B]"
                      >
                        <IoIosArrowBack className="h-4 w-4" />
                      </button>
                      <div className="flex-1">
                        <Typography variant="h4">Registry Asset Lookup</Typography>
                        <Typography variant="subtitle1" className="text-sm text-muted-foreground">
                      ...
                      <div className="grid grid-cols-1 2xl:grid-cols-2 gap-3 border-t pt-2 border-[#363638]">
                        <div className="mt-0 z-50">
                          ...
                          <SelectField
                            ...
                            name="registry"
                            isDisabled={isLoading || registryDataList?.length > 0}
                            value={registryOptions.find(
                              (option) => option.value === values.registry
                            )}
                            onChange={(selectedOption) => {
                              const newRegistryValue = selectedOption ? selectedOption.value : "";
                        ...
                        <div className="mt-0">
                          ...
                          <div className="flex">
                            <Input
                              ...
                              onChange={(e) => {
                                setFieldValue("reference", "");
                                setFieldValue("assetReferenceType", e.target.value);
                              }}
                              onBlur={handleBlur}
                              width="fit"
                              ...
                              disabled={
                                !values.registry || isLoading || registryDataList?.length > 0
                              }
                            />
                            <Input
                              id="ProjectID"
                              ...
                              onChange={(e) => {
                                setFieldValue("reference", "");
                                setFieldValue("assetReferenceType", e.target.value);
                                getRegistryAssets(values.registry, e.target.value);
                              }}
                              onBlur={handleBlur}
                              width="fit"
                              className="bg-transparent accent-tbase  border-transparent shadow-none focus-within:ring-transparent focus-within:border-transparent focus-within:outline-transparent"
                            ...
                            <Input
                              ...
                              name="assetReferenceType"
                              value="registryURL"
                              checked={values.assetReferenceType === "registryURL"}
                              onChange={(e) => {
                                setFieldValue("reference", "");
                                setFieldValue("assetReferenceType", e.target.value);
                              }}
                          ...
                          {touched.assetReferenceType && errors.assetReferenceType && (
                            <div className="text-red-500 text-sm mt-1">
                              {String(errors.assetReferenceType)}
                            </div>
                          )}
                        </div>
                      </div>
                            ...
                            <div className="flex rounded-lg">
                              ...
                              <div className="w-3/12">
                                <Button
                                  ...
                                  disabled={registryDataList.length > 0 || !!search}
                                >
                                  <span>
                  ...
                  {(registryDataList.length > 0 || search) && (
                    <LastSearchInfo
                      lastSearchCriteria={values as unknown as SearchCriteria}
                      resetForm={resetForm}
                    />
                  )}
                  {!selectedCarbonCreditDetails && !isCarbonCreditDetailsLoading && (
                    <>
                      <CarbonCreditList
                        carbonCredits={registryDataList as unknown as CarbonCredit[]}
                        onSelectCredit={(credit) =>
                          handleSelectCarbonCredit(credit, values.registry)
                        }
                        isLoading={isLoading}
                        onSearch={(query) => handleSearchQuery(query, values as FormValues)}
                      />
                      {registryDataListCount > 0 && (
                        <Pagination
                          ...
                          onPageChange={(page) => {
                            ...
                            if (
                              lastSearchCriteria &&
                              lastSearchCriteria.assetReferenceType === "Project ID"
                            ) {
                              searchRegistry(
                                lastSearchCriteria.registry.toLowerCase().replace(/ /g, "_"),
                                "projectID",
                                lastSearchCriteria.reference,
                                search,
                                page,
...
export default SearchAssetFromRegistry;
