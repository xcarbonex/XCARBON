...
interface TokenizationFormValues {
  id?: string;
  registry?: string;
  projectName: string;
  ...
  allowFactorization: string | boolean;
  fraction: string | number;
  files: unknown[];
  mintQuantity?: string | number;
}

interface RegistryFormsProps {
  onBackToList?: () => void;
  showBackToList?: boolean;
  resetParentForm?: React.MutableRefObject<{ resetForm: () => void } | null>;
}

const RegistryForms: React.FC<RegistryFormsProps> = ({
  onBackToList,
  ...
  const tokenizationInitialValues: TokenizationFormValues = {
    ...
    tokenSymbol: String(
      (selectedCarbonCreditDetails as unknown as Record<string, unknown>)?.tokenSymbol || ""
    ),
    listingPrice: String(
      (selectedCarbonCreditDetails as unknown as Record<string, unknown>)?.listingPrice || ""
    ),
    listingDuration: String(
      (selectedCarbonCreditDetails as unknown as Record<string, unknown>)?.listingDuration || ""
    ),
    allowFactorization: String(
      (selectedCarbonCreditDetails as unknown as Record<string, unknown>)?.allowFactorization || ""
    ),
    fraction: String(
      (selectedCarbonCreditDetails as unknown as Record<string, unknown>)?.fraction || ""
    ),
    files:
      ((selectedCarbonCreditDetails as unknown as Record<string, unknown>)?.files as unknown[]) ||
      [],
    mintQuantity: selectedCarbonCreditDetails?.quantity || "", // Add for FileUpload component
  };
  const handleTokenizationSubmit = async (values: TokenizationFormValues) => {
    await setMintedAssets(values as unknown as Parameters<typeof setMintedAssets>[0]);
    await togglePreview();
  ...
  const handleSaveAsDraft = async (values: TokenizationFormValues) => {
    await setDraft(values as unknown as Parameters<typeof setDraft>[0]);
    await togglePreview();
    await resetParentForm?.current?.resetForm();
    await navigate("/assets");
  ...
  const validationSchema = Yup.object({
    ...
    files: Yup.array()
      .length(2, "Exactly 2 files must be uploaded for Proof of Ownership/Retirement.")
  ...
  return (
    <>
      {carbonCreditDetailsError && (
        <div className="text-red-500 text-center mt-3">Error: {carbonCreditDetailsError}</div>
      )}
      {isCarbonCreditDetailsLoading && (
      ...
      {selectedCarbonCreditDetails && (
        <Form
          ...
          onSubmit={(values) => handleTokenizationSubmit(values as TokenizationFormValues)}
        >
                  ...
                  <div>
                    <div className=" dark:bg-[#141517] bg-[#FDFDFB]  border rounded-xl p-4 mt-2">
                      ...
                      <div>
                        ...
                        <div
                          className={clsx({
                            hidden: !selectedCarbonCreditDetails?.impactTags?.length,
                          })}
                        >
                          <div>
                            ...
                            <div className=" bg-[#4c666326] dark:bg-[#ffffff14] rounded-md">
                              <div className="py-3 pl-3">
                                <div className="flex flex-wrap gap-2">
                                  {selectedCarbonCreditDetails?.impactTags?.map((tag, index) => (
                                    <div
                                      key={index}
                          ...
                          <div className="grid mt-3 gap-3">
                            ...
                            <div className="flex gap-4">
                              <label htmlFor="factorization-yes">
                                <Input
                                  ...
                                  onChange={handleChange}
                                  suffix={<span className="text-[#949494] text-[20px]">Yes</span>}
                              ...
                              <label htmlFor="factorization-no">
                                <Input
                                  ...
                                  name="allowFactorization"
                                  value="false"
                                  checked={values.allowFactorization === "false"}
                                  onChange={handleChange}
                                  suffix={
                            ...
                            {values.allowFactorization == "true" && (
                              <div className="mt-4 p-4 border rounded bg-[#f5f5f5] dark:bg-[#222]">
                                <h2 className="text-lg font-semibold mb-2">Factorization</h2>
                                <div>
                    ...
                    <div className="flex justify-end gap-x-5">
                      <div>
                        <button
                          onClick={(e) => {
                            e.preventDefault();
                            handleSaveAsDraft(values as TokenizationFormValues);
                          }}
