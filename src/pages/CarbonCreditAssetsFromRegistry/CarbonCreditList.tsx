...
import { HiOutlineLocationMarker, HiOutlineCalendar, HiOutlineCube } from "react-icons/hi";
...
interface CarbonCreditListProps {
  ...
  onSelectCredit: (credit: CarbonCredit) => void;
  isLoading: boolean;
  onSearch?: (query: string) => void;
}

const CarbonCreditList: React.FC<CarbonCreditListProps> = ({
  carbonCredits = [],
  onSelectCredit,
  isLoading,
  onSearch = () => {},
  ...
  const [selectedProjects, setSelectedProjects] = useState<DropdownOption[]>([]); // State for multi-select dropdown
  const [selectedProjedType, setSelectedProjectedType] = useState<DropdownOption[]>([]);
  const [volume, setVolume] = useState<VolumeRange>({
    min: "",
    max: "",
  });
  const [selectedStatuses, setSelectedStatus] = useState<DropdownOption[]>([]);

  // Helper function to convert volume string to number
  const convertVolumeToFilter = (vol: VolumeRange) => ({
    min: vol.min ? Number(vol.min) : undefined,
    max: vol.max ? Number(vol.max) : undefined,
  });
  ...
  const handleProjectSelect = (selected: DropdownOption | DropdownOption[] | null) => {
    setSelectedProjects(Array.isArray(selected) ? selected : selected ? [selected] : []); // Update selectedProjects state
    // You might want to filter carbonCredits based on selected projects here
  };

  const handleVolumeSearch = async (setIsOpen: React.Dispatch<React.SetStateAction<boolean>>) => {
    let projectIds =
      selectedProjects.length > 0 ? selectedProjects.map((project) => project.value).join(",") : "";
    let projectTypeIds =
      selectedProjedType.length > 0 ? selectedProjedType.map((type) => type.value).join(",") : "";
    let assetType = selectedStatuses.reduce(
      (acc, item) => {
        acc[item.label.toLowerCase() as keyof typeof acc] = item.value as unknown as boolean;
        return acc;
      },
  ...
  const handleVolumeReset = async (setIsOpen: React.Dispatch<React.SetStateAction<boolean>>) => {
    setVolume({
      min: "",
      max: "",
    });
  ...
  const handleProjectType = (selected: DropdownOption | DropdownOption[] | null) => {
    setSelectedProjectedType(Array.isArray(selected) ? selected : selected ? [selected] : []);
  };

  const handleclick = () => {
    if (registryProjectList.length == 0) fetchRegistryProjectList(filteredPamaeter?.registry);
  };

  const handlePrjectSearch = async (
    _value: string,
  ...
  const handleProjectTypeSearch = async (
    _value: string,
    setIsOpen: React.Dispatch<React.SetStateAction<boolean>>
  ) => {
    let projectIds =
  ...
  const handleProjectTypeReset = async (
    setIsOpen: React.Dispatch<React.SetStateAction<boolean>>
  ) => {
    let projectIds =
  ...
  const handleResetProjectFilter = async (
    setIsOpen: React.Dispatch<React.SetStateAction<boolean>>
  ) => {
    let projectTypeIds =
      selectedProjedType.length > 0 ? selectedProjedType.map((type) => type?.value).join(",") : "";
  ...
  const handleRegistrAssetTypeFilter = async (
    _value: unknown,
    setIsOpen: React.Dispatch<React.SetStateAction<boolean>>
  ) => {
    let projectTypeIds =
    ...
    <>
      ...
      <div className="dark:bg-[#141517] bg-[#FDFDFB] border rounded-xl p-4 mt-2">
        <div className="border-b border-[#363638] pb-4 mb-4 flex justify-between items-center">
          <div>
            <h1 className="text-[24px] text-black dark:text-white">Carbon Credit Assets</h1>
        ...
        {filteredPamaeter?.registry && filteredPamaeter?.assetReferenceType === "projectID" && (
          <div className="grid grid-cols-5 gap-4 pb-4 mb-4 border-b border-[#363638]">
              ...
              /*registryProjectList.length > 0 && */ <div className="flex flex-col gap-2">
                ...
                <Dropdown
                  ...
                  onSearchApply={(val, setIsOpen) =>
                    handlePrjectSearch(Array.isArray(val) ? val.join(",") : String(val), setIsOpen)
                  }
                  ...
                  renderOption={({ option, isSelected, onSelect }) => {
                    ...
                    const isAllSelected = selectedProjects.length === registryProjectList.length;
                    return (
                      <div
            ...
            <div className="flex flex-col gap-2">
              ...
              <Dropdown
                options={[]}
                label={() => `Volume (min: ${volume.min || 0}, max: ${volume.max || 0})`}
                buttonClassName="w-full text-left flex items-center"
                dropdownClassName="w-full"
                  ...
                  <>
                    <div className="flex gap-3 p-1">
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          name="minVolume"
                          value={volume.min}
                          onChange={(e) => setVolume({ ...volume, min: e.target.value })}
                          variant="xs"
                          placeholder="Minimum"
                        />
                      </div>
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          name="maxVolume"
                          value={volume.max}
                          onChange={(e) => setVolume({ ...volume, max: e.target.value })}
                          variant="xs"
                          placeholder="Maximum"
                        />
                      </div>
                    ...
                    {parseInt(volume.max) < parseInt(volume.min) && (
                      <p className="text-red-500 text-sm px-1">
                        Invalid: Maximum value must be greater than Minimum Value.
                      </p>
                    )}
            ...
            {projectTypeList.length > 0 && (
              <div className="flex flex-col gap-2">
                ...
                <Dropdown
                  ...
                  renderOption={({ option, isSelected, onSelect }) => {
                    const isSelectAll = option.value === "_select_all_";
                    const isAllSelected = selectedProjedType.length === projectTypeList.length;
                    return (
                      <div
                        data-tooltip-id={"filter_val"}
                        data-tooltip-content={option.label}
            ...
            <div className="flex flex-col gap-2">
              ...
              <Dropdown
                ...
                onSelect={(option) =>
                  setSelectedStatus(Array.isArray(option) ? option : option ? [option] : [])
                }
        ...
        {!carbonCredits || (carbonCredits.length === 0 && !isLoading && <>{<NoDataFound />}</>)}
          ...
          <>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-3">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[14px] text-[#949494] dark:text-white/60">
                            Issuance Date:{" "}
                            {dayjs(credit?.created_at).format("MMMM DD, YYYY") || "N/A"}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <HiOutlineCube className="h-4 w-4 text-[#949494]" />
                          <span className="text-[14px] text-[#949494] dark:text-white/60">
                            Retirement Date:{" "}
                            {dayjs(credit?.certified_date).format("MMMM DD, YYYY") || "N/A"}
                          </span>
                        </div>
                      </div>
            ...
            <div className="mt-6 pt-4 border-t border-[#363638]">
              <p className="text-[12px] text-[#949494] dark:text-white/40 text-center">
                Click on any carbon credit to view details and proceed with tokenization
              </p>
            </div>
          </>
        )}
