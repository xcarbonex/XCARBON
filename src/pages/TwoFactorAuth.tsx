...
const TwoFactorAuth: React.FC<TwoFactorAuthProps> = ({ onSubmit, error }) => {
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    ...
    const otpArr = pastedData.slice(0, 6).split("");
    setOtp([...otpArr, ...Array(6 - otpArr.length).fill("")]);
  };

  const _handleResendOTP = (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
  ...
  return (
    ...
    >
      <div className="max-w-md w-full space-y-4 bg-secondary border text-tbase p-8 rounded-lg shadow-lg">
        ...
        <div>
          <h2 className="mt-1 text-center text-xl font-semibold">Two-Factor Authentication</h2>
          <p className="mt-2 text-center text-sm">
            Enter the 6-digit code from your authenticator app.
          </p>
          <p className="mt-1 text-center text-xs text-tbase-500">
            For your security, we require a one-time code from your authenticator app to complete
            sign in. This helps keep your account safe.
          </p>
        </div>
