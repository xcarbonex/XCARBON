...
interface WithdrawTokenizedCarbonCreditState {
  ...
  saveUserWithdrawAssets: (payload: WithdrawAssetInput) => Promise<void>;
  clearError: () => void;
}

const useWithdrawTokenizedCarbonCreditStore =
  create<WithdrawTokenizedCarbonCreditState>()(
    withDevtools(
      ...
      (set: any) => ({
        ...
        fetchAssetById: async (id: string) => {
          ...
          try {
            const response =
              await withdrawTokenizedCarbonCreditService.getAssetById(id);
            if (response.success) {
        ...
        saveUserWithdrawAssets: async (payload: WithdrawAssetInput) => {
          ...
          try {
            const response =
              await withdrawTokenizedCarbonCreditService.saveWithdrawAssets(
                payload
              );
            if (response.success) {
