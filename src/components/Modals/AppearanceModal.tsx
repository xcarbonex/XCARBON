...
const AppearanceModal: React.FC<AppearanceModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Appearance"
      className={clsx("w-full max-w-md p-6", "bg-secondary", "text-tbase", "border")}
    >
            ...
            return (
              ...
              >
                <div className="flex-1 flex items-center gap-3">
                  <Icon className={clsx("w-6 h-6", "text-black dark:text-white")} />
                  <div className="text-left">
                    <Typography
                      variant="body1"
                      className={clsx("font-medium", "text-black dark:text-white")}
                    >
                      {themeOption.name}
                    </Typography>
                ...
                {isSelected && <div className="w-3 h-3 rounded-full bg-[#C2A57B]"></div>}
              </button>
            );
          })}
        </div>
