...
interface BuyCarbonCreditModalProps {
  isOpen: boolean;
  onClose: () => void;
  creditData: CreditData;
}

const BuyCarbonCreditModal: React.FC<BuyCarbonCreditModalProps> = ({
  isOpen,
  onClose,
  creditData,
}) => {
  ...
  const walletBalances = useMemo<Record<string, number>>(
    () => ({
  ...
  useEffect(() => {
    ...
    } else if (totalCost > (walletBalances[paymentMethod?.value as string] || 0)) {
      setError(`Insufficient ${String(paymentMethod?.value).toUpperCase()} balance`);
    } else {
      setError("");
    }
  }, [quantity, pricePerCredit, availableQuantity, paymentMethod, totalCost, walletBalances]);
  ...
  const remainingBalance = (walletBalances[paymentMethod?.value as string] || 0) - totalCost;
  ...
  const handlePaymentForAssets = async () => {
    ...
    try {
      ...
      setTotalCost(0);
      setError("");
      setPaymentMethod({ value: "fiat", label: "Pay with Fiat (USD)" });
      onClose();
    }
  };
      ...
      <div className="space-y-6 mt-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800">
                  ...
                  <div>
                    ...
                    >
                      {formatCurrency(
                        walletBalances?.[paymentMethod?.value as string] || 0,
                        paymentMethod?.value === "xcb" ? "XCB" : "USD"
                      )}
                    </Typography>
                  </div>
                </div>
              ...
              <div className="p-3 bg-secondary/20">
                <div className="flex justify-between items-center">
                  ...
                  <Typography variant="subtitle2" className="font-medium">
                    {formatCurrency(totalCost || 0, paymentMethod?.value === "xcb" ? "XCB" : "USD")}
                  </Typography>
                </div>
              </div>
              <div
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {isLoading ? <Spinner size="sm" /> : "Confirm Purchase"}
            </button>
          </div>
        </form>
      </div>
