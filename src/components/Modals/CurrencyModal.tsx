...
const CurrencyModal: React.FC<CurrencyModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <Modal
      ...
      className={clsx("w-full max-w-md p-6")}
    >
          ...
          {currencies.map((currency) => (
            <button
              ...
              className={clsx(
                ...
                {
                  "hover:bg-[#4c666311] dark:hover:bg-[#ffffff1c]":
                    selectedCurrency === currency.code && currency.status,
                }
              )}
            >
              <p
                className={clsx(
                  "absolute top-1 right-1 bg-[#C2A57B] dark:bg-white dark:text-black text-bolder text-xs rounded-full px-1 hover:bg-opacity-90 text-white",
                  { hidden: currency.status }
                )}
              >
                Coming Soon
              </p>
              ...
              {selectedCurrency === currency.code && <FiCheck className="w-5 h-5 text-[#C2A57B]" />}
            </button>
          ))}
        </div>
