...
type PreviewPosition = "inside" | "below" | "left" | "right";
...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const isDragDisabled = files.length >= maxFiles;
  ...
  const handleChange = async (
    ...
    const newDuplicateNames: string[] = [];
    ...
    if (replaceIndex !== null) {
      ...
      if (selectedFile && selectedFile.size <= maxSize) {
        const newFiles = currentFiles.map((f, i) => (i === replaceIndex ? selectedFile : f));
      ...
      let updatedFiles = [...currentFiles, ...newUniqueFiles].slice(0, maxFiles);
      setFieldValue(name, updatedFiles);
    }

    // Simulate upload progress (only for newly added unique files, or after a successful replacement)
    if (
      (replaceIndex !== null && selectedFiles.length > 0 && selectedFiles[0].size <= maxSize) ||
      newUniqueFiles.length > 0
    ) {
      setProgress(0);
      const filesToProcessForProgress =
        replaceIndex !== null && selectedFiles.length > 0 ? selectedFiles : newUniqueFiles;
    ...
    if (newDuplicateNames.length > 0) {
      setDuplicateWarning(`Duplicate file(s) not added: ${newDuplicateNames.join(", ")}.`);
    } else {
      setDuplicateWarning("");
    }
  ...
  const handleRemove = (idx: number) => {
    ...
    if (
      ...
      newFiles.every(
        (f) =>
          !files.some((existing) => `${existing.name}-${existing.size}` === `${f.name}-${f.size}`)
      )
    ) {
      setDuplicateWarning("");
    }
      ...
      {files.map((file, idx) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <span className="text-sm text-tbase block truncate">{file.name}</span>
  ...
  const inputArea = (
    ...
    >
      <img src={icon || "/assets/certificate.svg"} alt="upload icon" className="mb-1 w-8 h-8" />
      ...
      <p className="text-xs font-medium text-[#949494] mt-1">
        {helperText || `Upload PDF, DOCX, or JPG (Max ${maxSize / (1024 * 1024)}MB)`}
      </p>
  ...
  return (
    ...
    >
      ...
      <div className={`flex ${previewPosition === "below" ? "flex-col" : "items-start gap-4"}`}>
      ...
      {duplicateWarning && <div className="text-yellow-500 text-xs mt-1">{duplicateWarning}</div>}
